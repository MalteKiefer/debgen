...
echo "\n";
  if (isset($_POST['debFast'])) {
    ...
    if (isset($_POST["backports"])) {
        $releaseBackports = ($_POST["release"] == "stable") ? "buster" : "stretch";
...
###### 3rd Party Binary Repos";
    ...
    for ($i=0;$i < count($_POST["3rdparties"]);$i++) {
        ...
        if ($rd3->repo_arch == "all" or $rd3->repo_arch == "") {
            ...
            if ($rd3->repo_url_src != '') {
                $newstring2 = $rd3->repo_url_src;
            }
        } else {
            $newstring=substr_replace($rd3->repo_url, "[arch=$rd3->repo_arch] ", 4, 0);
            if ($rd3->repo_url_src != '') {
                $newstring2 = substr_replace($rd3->repo_url_src, "[arch=$rd3->repo_arch] ", 8, 0);
            };
        }
        echo $newstring;
        if ($newstring2 != '') {
